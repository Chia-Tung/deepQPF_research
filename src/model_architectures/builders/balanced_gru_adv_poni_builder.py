...
class BalancedGRUAdvPoniBuilder(BaseBuilder):
    ...
    def prepare_components(self):
        ...
        self.prepare_encoder(num_channel, self._data_info["shape"]).prepare_decoder(
            num_channel_add
        ).prepare_discriminator(self._data_info["shape"]).prepare_loss_fn(
            self._loss_type
        ).prepare_dis_loss_fn()

    def prepare_encoder(
        self, n_channel: int, shape: tuple[int]
    ...
    def prepare_decoder(self, n_channel: int) -> BalancedGRUAdvPoniBuilder:
    ...
    def prepare_discriminator(self, shape: tuple[int]) -> BalancedGRUAdvPoniBuilder:
        self._discriminator = ma.Discriminator(shape, downsample=self._dis_d)
        return self

    def prepare_loss_fn(self, loss_type: str) -> BalancedGRUAdvPoniBuilder:
