...
class RainLoaderNc(BasicLoader):
    ...
    def inp_initial_time_fn(self, dt_list: list[datetime], ilen: int) -> list[datetime]:
        ...
        for idx, dt in reversed(list(enumerate(dt_list))):
            ...
            if dt - dt_list[idx - input_offset] == timedelta(
                minutes=input_offset * self.GRANULARITY
            ):
    ...
    def oup_initial_time_fn(
        ...
        while raw_idx < len(dt_list):
            ...
            if dt_list[raw_idx + target_offset] - dt_list[raw_idx] == timedelta(
                minutes=target_offset * self.GRANULARITY
            ):
    ...
    def load_input_data(
        ...
        data = []
        for time_offset in (
            timedelta(minutes=self.GRANULARITY * i) for i in range(ilen - 1, -1, -1)
        ):
    ...
    def set_lat_range(self):
        ...
        assert (
            np.sum(np.isin(self._lat_range, self._reader.INVALID_VALUE)) == 0
    ...
    def set_lon_range(self):
        ...
        assert (
            np.sum(np.isin(self._lon_range, self._reader.INVALID_VALUE)) == 0
